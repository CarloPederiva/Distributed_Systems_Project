...
contract("TasksContract", (accounts) => {
  ...
  it("get Tasks List", async () => {
    ...
    assert.equal(task.title, "Mercedez Benz");
    assert.equal(task.description, "White color, high power, safety at all times");
  ...
  it("task created successfully", async () => {
    const result = await this.tasksContract.createTask("some task two", "description two", "0.001");
